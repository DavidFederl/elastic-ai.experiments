...
def load_json_files(directory: Path) -> List[Dict]:
    ...
    def sort_key(file_path):
        ...
        if epoch_match:
            return (0, int(epoch_match.group(1)))  # (0, epoch_number) for numeric sorting
...
def generate_graphs(
    ...
    if not MATPLOTLIB_AVAILABLE:
        logger.error("Matplotlib is not installed. Please install it to generate graphs.")
    ...
    if data and isinstance(data[0], dict) and "original" in data[0] and "simulated" in data[0]:
