...
from src.nn.model.conv import conv_torch_gupta
from src.nn.training import Metrics, TrainingBuilder, set_initial_seed
...
def main(
    ...
    validation_ds = FashionMNIST(
        root="datasets", train=False, transform=transformations
    )
    validation_dl = get_dataloader(validation_ds)

    _, model = conv_torch_gupta(
