...
from src.nn.model import linear_v1_delta
...
def main(
    ...
    fraction_bits: Annotated[int, typer.Option(min=1)] = 4,
    ...
    training_dl = get_dataloader(
        training_ds,
        batch_size=len(training_ds) if batch_size == 0 else batch_size,
    )
    ...
    model_id, model = linear_v1_delta(
    ...
    generate_graphs(
        input_dir=log_dir.joinpath("metrics", "training"),
        output_dir=log_dir.joinpath("graphs", "training"),
    )
